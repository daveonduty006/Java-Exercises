...
public class Date
{
	...
	public static String validerDate(int jour, int mois, int an, boolean etat[]) {
		...
		if (an < anneActuelle) {
			etat[2]=false;
			message+="Année "+an+ " ne peut pas être inférieure à l'année actuelle, soit "+anneActuelle+"\n";
		}else {etat[2]=true;}
		return message;
	}
	...
	public void setAn(int an) {
		...
		/*int anneActuelle = dateActuelle.getYear();
		if (an < anneActuelle) {
			System.out.println("Année "+an+ " ne peut pas être inférieure à l'année actuelle, soit "+anneActuelle);
		}else {this.an = an;}*/
	}
